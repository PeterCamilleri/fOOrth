...
class TimeLibraryTester < Minitest::Test
  ...
  def test_some_time_duration_values
    ...
    foorth_equal('Duration .sec_per_day   3/2 * .to_duration .hours  ', [12])
    ...
    foorth_equal('Duration .sec_per_hour  3/2 * .to_duration .minutes', [30])
    ...
    foorth_equal('Duration .sec_per_min   3/2 * .to_duration .seconds', [30])
    ...
    foorth_equal('Duration .sec_per_year        .to_duration .as_years  ', [1])
    foorth_equal('Duration .sec_per_year  3/2 * .to_duration .as_years  ', [1.5])
    foorth_equal('Duration .sec_per_year  2   * .to_duration .as_years  ', [2])

    foorth_equal('Duration .sec_per_year        .to_duration .as_months ', [12])
    foorth_equal('Duration .sec_per_year  3/2 * .to_duration .as_months ', [18])
    foorth_equal('Duration .sec_per_year  2   * .to_duration .as_months ', [24])
    foorth_equal('Duration .sec_per_month       .to_duration .as_months ', [1])
    foorth_equal('Duration .sec_per_month 3/2 * .to_duration .as_months ', [1.5])
    foorth_equal('Duration .sec_per_month 2   * .to_duration .as_months ', [2])

    foorth_equal('Duration .sec_per_year        .to_duration .as_days   ', [365.2425])
    foorth_equal('Duration .sec_per_year  3/2 * .to_duration .as_days   ', [1.5*365.2425])
    foorth_equal('Duration .sec_per_year  2   * .to_duration .as_days   ', [2*365.2425])
    foorth_equal('Duration .sec_per_month       .to_duration .as_days   ', [365.2425/12])
    foorth_equal('Duration .sec_per_month 3/2 * .to_duration .as_days   ', [1.5*365.2425/12])
    foorth_equal('Duration .sec_per_month 2   * .to_duration .as_days   ', [365.2425/6])
    foorth_equal('Duration .sec_per_day         .to_duration .as_days   ', [1])
    foorth_equal('Duration .sec_per_day   3/2 * .to_duration .as_days   ', [1.5])
    foorth_equal('Duration .sec_per_day   2   * .to_duration .as_days   ', [2])

    foorth_equal('Duration .sec_per_year        .to_duration .as_hours  ', [8765.82])
    foorth_equal('Duration .sec_per_year  3/2 * .to_duration .as_hours  ', [13148.73])
    foorth_equal('Duration .sec_per_year  2   * .to_duration .as_hours  ', [17531.64])
    foorth_equal('Duration .sec_per_month       .to_duration .as_hours  ', [730.485])
    foorth_equal('Duration .sec_per_month 3/2 * .to_duration .as_hours  ', [1095.7275])
    foorth_equal('Duration .sec_per_month 2   * .to_duration .as_hours  ', [1460.97])
    foorth_equal('Duration .sec_per_day         .to_duration .as_hours  ', [24])
    foorth_equal('Duration .sec_per_day   3/2 * .to_duration .as_hours  ', [36])
    foorth_equal('Duration .sec_per_day   2   * .to_duration .as_hours  ', [48])
    foorth_equal('Duration .sec_per_hour        .to_duration .as_hours  ', [1])
    foorth_equal('Duration .sec_per_hour  3/2 * .to_duration .as_hours  ', [1.5])
    foorth_equal('Duration .sec_per_hour  2   * .to_duration .as_hours  ', [2])

    foorth_equal('Duration .sec_per_year        .to_duration .as_minutes', [525949.2])
    foorth_equal('Duration .sec_per_year  3/2 * .to_duration .as_minutes', [788923.8])
    foorth_equal('Duration .sec_per_year  2   * .to_duration .as_minutes', [1051898.4])
    foorth_equal('Duration .sec_per_month       .to_duration .as_minutes', [43829.1])
    foorth_equal('Duration .sec_per_month 3/2 * .to_duration .as_minutes', [65743.65])
    foorth_equal('Duration .sec_per_month 2   * .to_duration .as_minutes', [87658.2])
    foorth_equal('Duration .sec_per_day         .to_duration .as_minutes', [1440])
    foorth_equal('Duration .sec_per_day   3/2 * .to_duration .as_minutes', [2160])
    foorth_equal('Duration .sec_per_day   2   * .to_duration .as_minutes', [2880])
    foorth_equal('Duration .sec_per_hour        .to_duration .as_minutes', [60])
    foorth_equal('Duration .sec_per_hour  3/2 * .to_duration .as_minutes', [90])
    foorth_equal('Duration .sec_per_hour  2   * .to_duration .as_minutes', [120])
    foorth_equal('Duration .sec_per_min         .to_duration .as_minutes', [1])
    foorth_equal('Duration .sec_per_min   3/2 * .to_duration .as_minutes', [1.5])
    foorth_equal('Duration .sec_per_min   2   * .to_duration .as_minutes', [2])

    foorth_equal('Duration .sec_per_year        .to_duration .as_seconds', [31556952])
    foorth_equal('Duration .sec_per_year  3/2 * .to_duration .as_seconds', [47335428])
    foorth_equal('Duration .sec_per_year  2   * .to_duration .as_seconds', [63113904])
    foorth_equal('Duration .sec_per_month       .to_duration .as_seconds', [2629746])
    foorth_equal('Duration .sec_per_month 3/2 * .to_duration .as_seconds', [3944619])
    foorth_equal('Duration .sec_per_month 2   * .to_duration .as_seconds', [5259492])
    foorth_equal('Duration .sec_per_day         .to_duration .as_seconds', [86400])
    foorth_equal('Duration .sec_per_day   3/2 * .to_duration .as_seconds', [129600])
    foorth_equal('Duration .sec_per_day   2   * .to_duration .as_seconds', [172800])
    foorth_equal('Duration .sec_per_hour        .to_duration .as_seconds', [3600])
    foorth_equal('Duration .sec_per_hour  3/2 * .to_duration .as_seconds', [5400])
    foorth_equal('Duration .sec_per_hour  2   * .to_duration .as_seconds', [7200])
    foorth_equal('Duration .sec_per_min         .to_duration .as_seconds', [60])
    foorth_equal('Duration .sec_per_min   3/2 * .to_duration .as_seconds', [90])
    foorth_equal('Duration .sec_per_min   2   * .to_duration .as_seconds', [120])
    foorth_equal('                        1     .to_duration .as_seconds', [1])
    foorth_equal('                        3/2   .to_duration .as_seconds', [1.5])
    foorth_equal('                        2     .to_duration .as_seconds', [2])
  ...
  def test_some_duration_formatting
    ...
    foorth_equal('Duration .sec_per_year        .to_duration f"%o%$o"      ', ["0 months"])
    foorth_equal('Duration .sec_per_year  5/2 * .to_duration f"%3o%$o"     ', ["  6 months"])
    foorth_equal('Duration .sec_per_year  5/2 * .to_duration f"%-3o%$o"    ', ["6   months"])
    foorth_equal('Duration .sec_per_month       .to_duration f"%-3o%$o"    ', ["1   month"])
    foorth_equal('Duration .sec_per_month 5/2 * .to_duration f"%-3o%$o"    ', ["2   months"])

    foorth_equal('                            0 .to_duration f"%?3o%?$o"   ', [""])
    foorth_equal('Duration .sec_per_year        .to_duration f"%?3o%?$o"   ', [""])
    foorth_equal('Duration .sec_per_year  5/2 * .to_duration f"%?3o%?$o"   ', ["  6 months"])
    foorth_equal('Duration .sec_per_year  5/2 * .to_duration f"%?-3o%?$o"  ', ["6   months"])
    foorth_equal('Duration .sec_per_month       .to_duration f"%?-3o%?$o"  ', ["1   month"])
    foorth_equal('Duration .sec_per_month 5/2 * .to_duration f"%?-3o%?$o"  ', ["2   months"])
    ...
    foorth_equal('Duration .sec_per_year        .to_duration f"%4.1O%$O"   ', ["12.0 months"])
    foorth_equal('Duration .sec_per_year  5/2 * .to_duration f"%4.1O%$O"   ', ["30.0 months"])
    foorth_equal('Duration .sec_per_year  5/2 * .to_duration f"%-4.1O%$O"  ', ["30.0 months"])
    foorth_equal('Duration .sec_per_month       .to_duration f"%-4.1O%$O"  ', ["1.0  month"])
    foorth_equal('Duration .sec_per_month 5/2 * .to_duration f"%-4.1O%$O"  ', ["2.5  months"])
    ...
    foorth_equal('Duration .sec_per_year        .to_duration f"%d%$d"      ', ["0 days"])
    foorth_equal('Duration .sec_per_month       .to_duration f"%-3d%$d"    ', ["0   days"])
    foorth_equal('Duration .sec_per_month 5/2 * .to_duration f"%-3d%$d"    ', ["15  days"])
    foorth_equal('Duration .sec_per_day         .to_duration f"%-3d%$d"    ', ["1   day"])
    foorth_equal('Duration .sec_per_day     2 * .to_duration f"%-3d%$d"    ', ["2   days"])

    foorth_equal('                            0 .to_duration f"%?d%?$d"    ', [""])
    foorth_equal('Duration .sec_per_year        .to_duration f"%?d%?$d"    ', [""])
    foorth_equal('Duration .sec_per_month       .to_duration f"%?-3d%?$d"  ', [""])
    foorth_equal('Duration .sec_per_month 5/2 * .to_duration f"%?-3d%?$d"  ', ["15  days"])
    foorth_equal('Duration .sec_per_day         .to_duration f"%?-3d%?$d"  ', ["1   day"])
    foorth_equal('Duration .sec_per_day     2 * .to_duration f"%?-3d%?$d"  ', ["2   days"])

    foorth_equal('                            0 .to_duration f"%4.1D%$D"   ', [" 0.0 days"])
    foorth_equal('Duration .sec_per_year        .to_duration f"%4.1D%$D"   ', ["365.2 days"])
    foorth_equal('Duration .sec_per_month       .to_duration f"%-4.1D%$D"  ', ["30.4 days"])
